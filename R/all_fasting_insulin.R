...
{
    result_all <- aou.reader::lab_query(c("Insulin [Units/volume] in Serum or Plasma --fasting",
                       "Insulin [Units/volume] in Serum or Plasma --12 hours fasting"),
                        anchor_date_table,before,after)
    colnames(result_all) <- c("person_id","all_fasting_insulin_entry_date","all_fasting_insulin_value")
    .write_to_bucket(result_all,output_folder,"all_fasting_insulin")
}