...
#' @export
closest_fasting_glucose <- function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
    if (is.null(anchor_date_table))
    {
    stop("closest_fasting_glucose is not a primary variable and requires an anchor date table.")
    }
    lab_terms <- c("Fasting glucose [Mass/volume] in Serum or Plasma",
                   "Fasting glucose [Moles/volume] in Blood")
    ...
    result_all <- result_all[,.(closest_fasting_glucose_entry_date = measurement_date[1],
                                closest_fasting_glucose_value = value_as_number[1]),
                              .(person_id,anchor_date)]
    .write_to_bucket(result_all,output_folder,"closest_fasting_glucose")
}
