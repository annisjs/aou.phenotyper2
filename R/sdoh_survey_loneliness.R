...
sdoh_survey_loneliness <- function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  ...
  lonely_agg <- lonely_dat[, .(sdoh_survey_loneliness_score = mean(item_score, na.rm = T),
                               sdoh_survey_loneliness_date = survey_date[1],
                               n_responses = length(which(!is.na(item_score)))),
                            .(person_id)]
  lonely_agg <- lonely_agg[n_responses == 8]
  lonely_agg[, n_responses := NULL]
