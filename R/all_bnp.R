...
all_bnp <- function(output_folder, anchor_date_table = NULL, before = NULL, after = NULL)
{
  ...
  # Match the "all_hdl" pattern: standardize output column names and write
  colnames(result_all) <- c("person_id", "all_bnp_entry_date", "all_bnp_value")

  .write_to_bucket(result_all, output_folder, "all_bnp")
}