...
#' @export
insomnia_expanded <- function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
    ...
    result_all <- result_all[,.(insomnia_expanded_entry_date = min(condition_start_date),
                                insomnia_expanded_status = length(condition_start_date) > 0),
                              .(person_id)]
    .write_to_bucket(result_all,output_folder,"insomnia_expanded")
}
