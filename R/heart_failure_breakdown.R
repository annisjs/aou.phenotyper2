...
#' @return output_folder/heart_failure_breakdown.csv
#' @details At least 1 inpatient or 2 outpatient ICD codes: Result also includes inpatient and outpatient statuses indicating if either or both classifying conditions are satisfied.
#'
#' ICD9: "425","425.%","428","428.%",
#'
#' ICD10: "I42","I42.%","I50","I50.%"
#' @export
heart_failure_breakdown <- function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  ...
  # Save data to bucket
  hf_counts <- hf_counts[,c("person_id","heart_failure_entry_date","hf_inpt_status", "hf_outpt_status", "heart_failure_status")]
  hf_counts <- hf_counts[heart_failure_status == TRUE]
  .write_to_bucket(hf_counts,output_folder,"heart_failure_breakdown")
}