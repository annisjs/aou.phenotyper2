...
#' @export
closest_height <- function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
    ...
    result_all <- result_all[,.(closest_height_entry_date = measurement_date[1],
                                closest_height_value = height[1],
                                closest_height_unit = unit_concept_name[1]),.(person_id,anchor_date)]
    .write_to_bucket(result_all,output_folder,"closest_height")
}
