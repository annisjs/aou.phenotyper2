...
#' @export
closest_hdl <- function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
    if (is.null(anchor_date_table))
    {
    stop("closest_hdl is not a primary variable and requires an anchor date table.")
    }
    lab_terms <- "Cholesterol in HDL [Mass/volume] in Serum or Plasma"
    ...
    result_all <- result_all[,.(closest_hdl_entry_date = measurement_date[1],
                                closest_hdl_value = value_as_number[1]),
                            .(person_id,anchor_date)]
    .write_to_bucket(result_all,output_folder,"closest_hdl")
}
