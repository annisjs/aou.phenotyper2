...
anti_hypertensives <- function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
    ...
    result_all <- bp_meds[,.(med_status = length(drug_exposure_start_date) > 0,
                                med_entry_date = min(drug_exposure_start_date)),
                                .(person_id)]
    result_all$med_status[is.na(result_all$med_status)] <- FALSE
    result_all <- result_all[,.(anti_hypertensives_entry_date = med_entry_date,
                                anti_hypertensives = med_status),.(person_id)]
