...
closest_bp <-  function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
    if (is.null(anchor_date_table))
    {
    stop("closest_bp is not a primary variable and requires an anchor date table.")
    }  
    result_all <- aou.reader::bp_query(anchor_date_table,before,after)
    ...
    result_all <- result_all[,.(closest_bp_entry_date = measurement_date[1],
                                ...
                                closest_bp_diastolic = bp_diastolic[1]),
                            .(person_id,anchor_date)]
    .write_to_bucket(result_all,output_folder,"closest_bp")
}
