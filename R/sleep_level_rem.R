...
#' @export
sleep_level_rem <- function(output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
    sleep_terms <- "level = 'rem'"
    result <- aou.reader::sleep_level_query(sleep_terms,"all",anchor_date_table,before,after)
    colnames(result) <- c('person_id',
                        'sleep_level_rem_date',
